...
class PaperAddReq(BaseModel):
    ...
    name: str
    size: int
    content: Optional[str] = ""       # 检索导入的 PDF 传空字符串，不存 base64
...
def add_paper(req: PaperAddReq, user: User = Depends(current_user), db: Session = Depends(get_db)):
    is_search_pdf = req.from_search and req.file_type == "pdf"

    # 安全截断：检索 PDF 的 content 为空，普通论文截断到 80000 字符
    safe_content = (req.content or "")[:80000] if not is_search_pdf else ""
    # 预览：检索 PDF 用标题，普通论文用内容前 300 字
    safe_preview = (req.meta.get("title", req.name) if req.meta else req.name)[:300] \
                   if is_search_pdf else safe_content[:300]

    # 按 ID 查重（同一条记录更新）
    existing = db.query(Paper).filter_by(id=req.id, user_id=user.id).first()
    if existing:
        # 检索 PDF 只更新 meta，不覆盖 content（防止把已有内容清空）
        if not is_search_pdf:
    ...
    # 同名检测：只对普通上传论文做同名合并，检索 PDF 不参与（避免覆盖真实论文）
    if not is_search_pdf:
