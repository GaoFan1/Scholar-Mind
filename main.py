...
def add_paper(req: PaperAddReq, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if existing:
        existing.content = req.content[:80000]
        existing.preview = req.content[:300]
    ...
    if same_name:
        same_name.content = req.content[:80000]
        same_name.preview = req.content[:300]
        db.commit()
    ...
    paper = Paper(
        ...
        preview=req.content[:300],
        meta=req.meta,
        from_search=req.from_search or False,
    )
    db.add(paper)
    db.commit()
...
@app.delete("/api/library/{paper_id:path}")
...
def delete_paper_safe(req: DeletePaperReq, user: User = Depends(current_user), db: Session = Depends(get_db)):
    """通过POST body删除，避免id含特殊字符时URL路径解析问题"""
...
@app.delete("/api/notes/{paper_id:path}")
...
@app.get("/api/cache/{paper_id:path}/{cache_type}")
...
@app.delete("/api/cache/{paper_id:path}/{cache_type}")
...
@app.delete("/api/cache/{paper_id:path}")
